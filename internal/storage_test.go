...
func TestStorage_AcquireBookID(t *testing.T) {
	...
	s, err := NewStorage(1, 10000, file)
	if err != nil {
		t.Errorf("Error in creating storage: %v", err)
	}
...
func TestStorage_SaveBookID(t *testing.T) {
	...
	if bookID != 501 {
		t.Errorf("Error in acquire book id from storage file. Book id should be %d, but it's %d", 501, bookID)
	}
}
